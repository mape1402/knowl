...
builder.Services.AddKnOwlAspNetCore(options =>
{
...
builder.Services.AddKnOwlPigeon(options =>
{
...
builder.Services.AddKnOwlDashboard(options =>
{
...
app.MapGet("/", () => Results.Ok(new
{
    ...
    endpoints = new[]
    {
        ...
        "GET /knowl opens the live dashboard. Login with admin / secret."
    }
}));

app.MapPost("/orders/inline", async (
    CreateOrderRequest request,
    IKnOwlOperationService operations,
...
app.MapPost("/orders/deferred", async (
    ...
    CancellationToken cancellationToken) =>
{
    var open = await http.OpenKnOwlAsync(
        inbox,
        request,
...
static IResult ToHttpResult(KnOwlOperationResult<OrderSnapshot> result)
    => result.State switch
    {
        KnOwlOperationExecutionState.Executed => Results.Created(
            $"/orders/{result.Result.Id}",
            OrderAcceptedResponse.From(result.Result, result.EffectiveIdempotencyKey, result.Context.Entry.Id)),
        KnOwlOperationExecutionState.Deferred => Results.Accepted(
            $"/inbox/{result.Context.Entry.Id}",
            DeferredAcceptedResponse.From(result.EffectiveIdempotencyKey, result.Context.Entry.Id)),
        KnOwlOperationExecutionState.Replayed when result.Result is not null => Results.Ok(
...
/// <summary>
/// Declared KnOwl operation used by HTTP endpoints.
/// </summary>
public sealed class CreateOrderOperation : KnOwlOperation<CreateOrderRequest, OrderSnapshot>
{
    /// <inheritdoc />
...
/// <param name="InboxEntryId">The KnOwl inbox entry id.</param>
...
/// <summary>
/// Response returned when KnOwl rejects duplicate or conflicting work.
/// </summary>

...
/// <summary>
/// ASP.NET Core middleware that opens KnOwl inbox contexts from HTTP idempotency headers.
/// </summary>
public sealed class KnOwlMiddleware
{
    private readonly RequestDelegate _next;
    private readonly KnOwlAspNetCoreOptions _options;

    /// <summary>
    /// Initializes a new instance of the <see cref="KnOwlMiddleware"/> class.
    /// </summary>
    /// <param name="next">The next request delegate.</param>
    /// <param name="options">The middleware options.</param>
    public KnOwlMiddleware(RequestDelegate next, IOptions<KnOwlAspNetCoreOptions> options)
    {
    ...
    private InboxCompletion CreateCompletion(HttpContext httpContext, MemoryStream capturedBody)
    {
        ...
        if (capturedBody.Length > _options.MaxReplayBodyBytes)
            completion.Metadata["knowl:http:body-too-large"] = "true";

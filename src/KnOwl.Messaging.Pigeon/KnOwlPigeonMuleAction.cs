...
/// </summary>
[MuleAction(KnOwlPigeonMuleActionKeys.Consume)]
public sealed class KnOwlPigeonMuleAction : KnOwlMuleAction<PigeonConsumeEnvelope>
{
    private readonly IPigeonConsumerInvoker _consumerInvoker;

    /// <summary>
    /// Initializes a new instance of the <see cref="KnOwlPigeonMuleAction"/> class.
    /// </summary>
    /// <param name="inbox">The inbox service.</param>
    /// <param name="consumerInvoker">The Pigeon consumer invoker.</param>
    public KnOwlPigeonMuleAction(IInboxService inbox, IPigeonConsumerInvoker consumerInvoker)
        : base(inbox)
    {
    ...
    protected override async ValueTask ExecuteInboxAsync(
        KnOwlMuleActionContext<PigeonConsumeEnvelope> context,
    ...
    private static SemanticVersion ResolveVersion(PigeonConsumeEnvelope envelope)
    {
        if (envelope.Metadata is not null &&
            envelope.Metadata.TryGetValue(KnOwlPigeonMetadataNames.Version, out var rawVersion) &&

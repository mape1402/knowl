...
/// <summary>
/// KnOwl outbox publisher that delivers persisted envelopes through Pigeon.
/// </summary>
public sealed class KnOwlPigeonOutboxPublisher : IOutboxTransportPublisher
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="KnOwlPigeonOutboxPublisher"/> class.
    /// </summary>
    public KnOwlPigeonOutboxPublisher(
    ...
    private PigeonPublishEnvelope DeserializePublishEnvelope(OutboxEnvelope envelope)
    {
        ...
        if (payloadType == typeof(KnOwlPigeonOutboxPayload))
        {
            var legacyPayload = (KnOwlPigeonOutboxPayload)_outboxSerializer.Deserialize(
                envelope.Payload,
                typeof(KnOwlPigeonOutboxPayload));

            return CreateLegacyEnvelope(envelope, legacyPayload);
        }

        throw new InvalidOperationException(
            $"KnOwl Pigeon outbox envelope payload type '{envelope.PayloadType}' is not supported.");
    }
    ...
    private static PigeonPublishEnvelope CreateLegacyEnvelope(
        OutboxEnvelope envelope,
        KnOwlPigeonOutboxPayload payload)
        => new()
        {
            Transport = envelope.Transport,
            Topic = payload.Topic,
            Version = ResolveMetadata(envelope, KnOwlPigeonMetadataNames.Version),

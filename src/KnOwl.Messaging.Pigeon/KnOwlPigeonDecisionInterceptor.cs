...
/// </summary>
public sealed class KnOwlPigeonDecisionInterceptor : IConsumeDecisionInterceptor
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="KnOwlPigeonDecisionInterceptor"/> class.
    /// </summary>
    public KnOwlPigeonDecisionInterceptor(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (open.OpenResult.Entry?.ExecutionMode == InboxExecutionMode.Deferred)
        {
            ...
            envelope.Metadata[KnOwlPigeonMetadataNames.Version] = version.ToString();
            AttachEffectiveKey(envelope.Metadata, open.EffectiveIdempotencyKey);

            await _scheduler.EnqueueCurrentAsync(
                KnOwlPigeonMuleActionKeys.ConsumeKey,
                envelope,
            ...
            return new PigeonConsumeDecisionResult(
                ...
                "KnOwl scheduled the Pigeon consumer for deferred execution.")
            {
    ...
    private PigeonConsumeDecisionResult MapDecision(InboxDecision decision)
    {
        ...
        return new PigeonConsumeDecisionResult(
            pigeonDecision,
            $"KnOwl inbox decision '{decision.Action}' for state '{decision.State}'.")
        {
    ...
    private static void AttachEffectiveKey(IDictionary<string, string> metadata, string key)
    {
        if (!string.IsNullOrWhiteSpace(key))
            metadata[KnOwlMuleMetadata.IdempotencyKey] = key;
    }
    ...
    private static IReadOnlyDictionary<string, string> CreateDecisionMetadata(string key)
    {
        ...
        return new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            [KnOwlMuleMetadata.IdempotencyKey] = key
        };
    }
}

...
public sealed class DefaultOutboxService : IOutboxService
{
    ...
    private readonly IKnOwlEventPublisher _events;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await PublishEventAsync(KnOwlEventNames.OutboxEnqueued, envelope, cancellationToken);

        var schedulers = _schedulers.ToArray();
        if (schedulers.Length == 0 && _options.RequireDeferredScheduler)
            throw new InvalidOperationException("KnOwl outbox requires an IOutboxDeferredScheduler. Add KnOwl.Mule to enable durable outbox publication.");

        foreach (var scheduler in schedulers)
        {
            await scheduler.ScheduleAsync(envelope, cancellationToken);
            await PublishEventAsync(KnOwlEventNames.DeferredScheduled, envelope, cancellationToken);
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await PublishEventAsync(KnOwlEventNames.OutboxPublishing, envelope, cancellationToken);
            await PublishEventAsync(KnOwlEventNames.DeferredStarted, envelope, cancellationToken);
            ...
            await PublishEventAsync(KnOwlEventNames.OutboxPublished, envelope, cancellationToken);
            await PublishEventAsync(KnOwlEventNames.DeferredCompleted, envelope, cancellationToken);
            return OutboxPublishResult.Success;
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        await PublishEventAsync(KnOwlEventNames.OutboxFailed, envelope, cancellationToken);
        await PublishEventAsync(KnOwlEventNames.DeferredFailed, envelope, cancellationToken);
    }
    ...
    private IOutboxTransportPublisher ResolvePublisher(string transport)
    {
        ...
        return publisher ?? throw new InvalidOperationException(
            $"No KnOwl outbox publisher is registered for transport '{transport}'.");
    }

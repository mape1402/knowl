    ...
    where TDbContext : DbContext
{
    ...
    private readonly IKnOwlDbContextFactory<TDbContext> _dbContextFactory;

    /// <summary>
    /// Initializes a new instance of the <see cref="EntityFrameworkInboxStore{TDbContext}"/> class.
    /// </summary>
    /// <param name="dbContextFactory">The DbContext factory used to create short-lived KnOwl contexts.</param>
    internal EntityFrameworkInboxStore(IKnOwlDbContextFactory<TDbContext> dbContextFactory)
    {
    ...
    public EntityFrameworkInboxStore(IDbContextFactory<TDbContext> dbContextFactory)
        : this(new KnOwlDbContextFactoryAdapter<TDbContext>(dbContextFactory))
    {
    }
    ...
    public async ValueTask<InboxOpenResult> TryOpenAsync(InboxEntry entry, CancellationToken cancellationToken = default)
    {
        ...
        dbContext.Set<KnOwlInboxEntryRecord>().Add(record);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await using var dbContext = _dbContextFactory.CreateDbContext();
        var records = dbContext.Set<KnOwlInboxEntryRecord>().AsNoTracking().AsQueryable();
    ...
    private static async Task<KnOwlInboxEntryRecord> FindByBusinessKeyAsync(
        TDbContext dbContext,
        InboxEntry entry,
        CancellationToken cancellationToken)
        => await dbContext.Set<KnOwlInboxEntryRecord>()
            .AsNoTracking()
            .SingleAsync(
    ...
    private static async Task<KnOwlInboxEntryRecord> FindByIdAsync(
        TDbContext dbContext,
        Ulid entryId,
        CancellationToken cancellationToken)
        => await dbContext.Set<KnOwlInboxEntryRecord>()
    ...
    private static InboxOpenResult ResolveDuplicate(KnOwlInboxEntryRecord existing, InboxEntry incoming)
    {
    ...
    private static KnOwlInboxEntryRecord ToRecord(InboxEntry entry)
        => new()
        {
    ...
    private static InboxEntry ToEntry(KnOwlInboxEntryRecord record)
        => new()
        {

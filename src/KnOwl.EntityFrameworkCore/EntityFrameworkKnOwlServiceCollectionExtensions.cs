...
/// <summary>
/// Provides dependency injection registration for the KnOwl Entity Framework Core store.
/// </summary>
public static class EntityFrameworkKnOwlServiceCollectionExtensions
{
    /// <summary>
        ...
        where TDbContext : DbContext
    {
        ...
        services.AddKnOwlDbContextOptions<TDbContext>();
        services.TryAddScoped<IKnOwlDbContextFactory<TDbContext>, KnOwlDbContextFactory<TDbContext>>();
        services.AddScoped<EntityFrameworkInboxStore<TDbContext>>(provider =>
            new EntityFrameworkInboxStore<TDbContext>(
                provider.GetRequiredService<IKnOwlDbContextFactory<TDbContext>>()));
    ...
    private static IServiceCollection AddKnOwlDbContextOptions<TDbContext>(this IServiceCollection services)
        where TDbContext : DbContext
    {
        ...
        if (descriptor is null)
        {
            throw new InvalidOperationException(
                $"DbContext '{typeof(TDbContext).Name}' must be registered before enabling KnOwl EF storage.");
        }

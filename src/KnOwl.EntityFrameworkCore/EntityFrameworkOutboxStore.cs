    ...
    where TDbContext : DbContext
{
    ...
    internal EntityFrameworkOutboxStore(IKnOwlDbContextFactory<TDbContext> dbContextFactory)
    {
    ...
    public async ValueTask AddAsync(OutboxEnvelope envelope, CancellationToken cancellationToken = default)
    {
        ...
        dbContext.Set<KnOwlOutboxEnvelopeRecord>().Add(ToRecord(envelope));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await using var dbContext = _dbContextFactory.CreateDbContext();
        var records = dbContext.Set<KnOwlOutboxEnvelopeRecord>().AsNoTracking().AsQueryable();
    ...
    private static async Task<KnOwlOutboxEnvelopeRecord> FindByIdAsync(
        TDbContext dbContext,
        Ulid envelopeId,
        CancellationToken cancellationToken)
        => await dbContext.Set<KnOwlOutboxEnvelopeRecord>()
    ...
    private static KnOwlOutboxEnvelopeRecord ToRecord(OutboxEnvelope envelope)
        => new()
        {
    ...
    private static OutboxEnvelope ToEnvelope(KnOwlOutboxEnvelopeRecord record)
    {

...
/// <summary>
/// Provides endpoint mapping extensions for the KnOwl dashboard.
/// </summary>
public static class KnOwlDashboardEndpointRouteBuilderExtensions
{
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);
    private const string ProtectorPurpose = "KnOwl.AspNetCore.Dashboard.Authentication";

    /// <summary>
    /// Maps the KnOwl dashboard endpoints.
    /// </summary>
    ...
    public static IEndpointRouteBuilder MapKnOwlDashboard(
        this IEndpointRouteBuilder endpoints,
        string pattern = "/knowl")
    {
        ArgumentNullException.ThrowIfNull(endpoints);
        ValidateDashboardConfiguration(endpoints.ServiceProvider);

        var group = endpoints.MapGroup(string.IsNullOrWhiteSpace(pattern) ? "/knowl" : pattern.TrimEnd('/'));
    ...
    private static async Task<IResult> HandleIndexAsync(HttpContext context)
    {
        ...
        if (user is null)
        {
            ...
            return options.Authentication.Mode == KnOwlDashboardAuthenticationMode.AspNetCoreAuthentication
                ? Results.Unauthorized()
                : Results.Content(KnOwlDashboardHtml.Login, "text/html; charset=utf-8");
        }

        return Results.Content(KnOwlDashboardHtml.Dashboard, "text/html; charset=utf-8");
    }
    ...
    private static async Task<IResult> HandleLoginAsync(HttpContext context)
    {
        ...
        if (options.Authentication.Mode == KnOwlDashboardAuthenticationMode.AspNetCoreAuthentication)
        ...
        KnOwlDashboardUser user = null;

        if (options.Authentication.Mode == KnOwlDashboardAuthenticationMode.RootUser)
        {
            user = AuthenticateRootUser(options, login);
        }
        else
        {
            var authenticator = context.RequestServices.GetRequiredService<IKnOwlDashboardAuthenticator>();
            var result = await authenticator.AuthenticateAsync(
                new KnOwlDashboardAuthRequest
                {
                    HttpContext = context,
    ...
    private static async Task<IResult> HandleStateAsync(HttpContext context)
    {
        ...
        var sink = context.RequestServices.GetRequiredService<IKnOwlEventSink>();
    ...
    private static async Task HandleEventStreamAsync(HttpContext context)
    {
        ...
        var sink = context.RequestServices.GetRequiredService<IKnOwlEventSink>();
        var channel = Channel.CreateUnbounded<KnOwlEvent>();
    ...
    private static async Task WriteEventAsync(HttpContext context, KnOwlEvent @event)
    {
        var json = JsonSerializer.Serialize(@event, JsonOptions);
        await context.Response.WriteAsync("event: knowl\n", context.RequestAborted);
        await context.Response.WriteAsync($"data: {json}\n\n", context.RequestAborted);
        await context.Response.Body.FlushAsync(context.RequestAborted);
    }

    private static async Task<KnOwlDashboardUser> AuthenticateAsync(HttpContext context)
    {
        var options = GetOptions(context);
        ...
        if (options.Authentication.Mode == KnOwlDashboardAuthenticationMode.AspNetCoreAuthentication)
        {
            ...
            var user = new KnOwlDashboardUser
            {
        ...
        if (options.Authentication.Mode == KnOwlDashboardAuthenticationMode.Custom)
        {
            var authenticator = context.RequestServices.GetRequiredService<IKnOwlDashboardAuthenticator>();
            var result = await authenticator.AuthenticateAsync(
                new KnOwlDashboardAuthRequest { HttpContext = context },
                context.RequestAborted);
    ...
    private static KnOwlDashboardUser AuthenticateRootUser(
        KnOwlDashboardOptions options,
        LoginRequest login)
    {
    ...
    private static KnOwlDashboardUser TryReadCookie(
        HttpContext context,
        KnOwlDashboardOptions options)
    {
    ...
    {
        var options = services.GetRequiredService<IOptions<KnOwlDashboardOptions>>().Value;
        if (options.Authentication.Mode == KnOwlDashboardAuthenticationMode.RootUser)
        {
            var root = options.Authentication.RootUser;
                ...
                string.IsNullOrWhiteSpace(root.Password) && string.IsNullOrWhiteSpace(root.PasswordHash))
            {
                throw new InvalidOperationException(
                    "KnOwl dashboard root user is not configured. Configure a root username and password, or choose ASP.NET Core/custom authentication.");
            }
        }

        if (options.Authentication.Mode == KnOwlDashboardAuthenticationMode.Custom &&
            services.GetService<IKnOwlDashboardAuthenticator>() is null)
        {
            throw new InvalidOperationException(
                "KnOwl dashboard custom authentication requires an IKnOwlDashboardAuthenticator registration.");
        }
    }

    private static void SetCurrentUser(HttpContext context, KnOwlDashboardUser user)
        => context.Items[KnOwlDashboardUserAccessor.ItemKey] = user;

    private static KnOwlDashboardOptions GetOptions(HttpContext context)
        => context.RequestServices.GetRequiredService<IOptions<KnOwlDashboardOptions>>().Value;

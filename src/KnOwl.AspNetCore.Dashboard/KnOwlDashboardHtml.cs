...
internal static class KnOwlDashboardHtml
{
...
<body>
  ...
  <script>
    ...
    stream.addEventListener('knowl', e => { state.events.push(JSON.parse(e.data)); state.events = state.events.slice(-300); render(); });

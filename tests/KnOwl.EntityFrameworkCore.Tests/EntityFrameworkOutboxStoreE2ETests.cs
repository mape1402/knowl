...
namespace KnOwl.EntityFrameworkCore.Tests;
...
public sealed class EntityFrameworkOutboxStoreE2ETests
{
    ...
    private static ServiceProvider CreateProvider(string connectionString)
    {
        ...
        services.AddDbContext<TestKnOwlDbContext>(options => options.UseSqlServer(connectionString));
        services
            .AddKnOwl()
            .UseEntityFramework<TestKnOwlDbContext>();
    ...
    {
        using var scope = provider.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<TestKnOwlDbContext>();

        Assert.NotNull(dbContext.Model.FindEntityType(typeof(KnOwlInboxEntryRecord)));
        Assert.NotNull(dbContext.Model.FindEntityType(typeof(KnOwlOutboxEnvelopeRecord)));
    ...
    private static string CreateIsolatedConnectionString()
    {
        var builder = new SqlConnectionStringBuilder(
            Environment.GetEnvironmentVariable("KNOWL_SQLSERVER")
            ?? "Server=localhost,11434;Database=KnOwlTests;User Id=sa;Password=KnOwl_12345!;TrustServerCertificate=True;Encrypt=False");

        builder.InitialCatalog = $"KnOwlTests_{Environment.Version.Major}_{Ulid.NewUlid()}";
    ...
    private sealed class TestKnOwlDbContext : DbContext
    {
        public TestKnOwlDbContext(DbContextOptions<TestKnOwlDbContext> options)
            : base(options)
        {
        }
    }

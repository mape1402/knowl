...
public sealed class OutboxMuleSchedulerTests
{
    ...
    public async Task ScheduleAsync_enqueues_mule_action_with_outbox_metadata()
    {
        ...
        KnOwlOutboxEnvelopeReference capturedPayload = null;
        mule.EnqueueAsync(
                Arg.Do<ActionKey>(key => capturedKey = key),
                Arg.Do<KnOwlOutboxEnvelopeReference>(payload => capturedPayload = payload),
        ...
        var scheduler = new KnOwlOutboxMuleScheduler(mule);
        await scheduler.ScheduleAsync(envelope);

        Assert.Equal(KnOwlOutboxMuleActionKeys.PublishKey, capturedKey);
        ...
        Assert.Equal(envelope.Id.ToString(), capturedOptions.Metadata[KnOwlMuleMetadata.OutboxEnvelopeId]);
    }

...
namespace KnOwl.Mule.Tests;

public sealed class InboxMuleSchedulerTests
{
    private static readonly ActionKey Key = ActionKey.From("tests.knowl.deferred.v1");
    ...
    public async Task EnqueueCurrentAsync_attaches_inbox_metadata_to_mule_action()
    {
        ...
        Assert.Equal(opened.Entry.Id.ToString(), capturedOptions.Metadata[KnOwlMuleMetadata.InboxEntryId]);
        Assert.Equal("order-1", capturedOptions.Metadata[KnOwlMuleMetadata.IdempotencyKey]);
    }

    [Fact]
    public async Task KnOwlMuleAction_continues_context_and_completes_inbox_entry()
    {
    ...
    [Fact]
    public async Task KnOwlMuleAction_fails_inbox_entry_when_execution_fails()
    {
        using var provider = CreateServiceProvider();
    ...
    public async Task Hosted_mule_worker_executes_deferred_inbox_entry_end_to_end()
    {
        ...
        Assert.Equal(entryId.ToString(), metadata[KnOwlMuleMetadata.InboxEntryId]);
        Assert.Equal(entryId, probe.EntryIds.Single());
    ...
    public async Task Hosted_mule_worker_executes_deferred_declared_operation_end_to_end()
    {
        ...
        using (var scope = host.Services.CreateScope())
        {
            var result = await scope.ServiceProvider
                .GetRequiredService<IKnOwlOperationService>()
    ...
    private static IHost CreateOperationHost()
        => Host.CreateDefaultBuilder()
            .ConfigureServices(services =>
            {
                ...
                services.AddKnOwl(options => options.DefaultExecutionMode = InboxExecutionMode.Deferred).UseInMemory();
                services.AddKnOwlMule();
                services.AddMule(mule => mule
                    .UseInMemory()
                    .AddActionsFromAssemblyContaining<KnOwlOperationMuleAction>());
            })
            .Build();
        ...
        DeferredPayload payload)
    {
        var action = new DurableAction
        {
            ...
            {
                [KnOwlMuleMetadata.InboxEntryId] = entryId.ToString(),
                [KnOwlMuleMetadata.IdempotencyKey] = payload.Id
            })
        };
    ...
    [MuleAction("tests.knowl.deferred.v1")]
    private sealed class CaptureDeferredPayloadAction : KnOwlMuleAction<DeferredPayload>
    {
        ...
        protected override ValueTask ExecuteInboxAsync(
            KnOwlMuleActionContext<DeferredPayload> context,
    ...
    private sealed class CreateDeferredOrderOperation : KnOwlOperation<DeferredOrderRequest, DeferredOrderResult>
    {

...
using KnOwl.Messaging.Pigeon;

namespace KnOwl.Messaging.Tests;

public sealed class KnOwlPigeonOutboxTests
{
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);

    [Fact]
    public async Task PublishDecisionInterceptor_persists_pigeon_publish_envelope_in_knowl_outbox()
    {
        using var provider = CreateProvider();
        using var scope = provider.CreateScope();
        var interceptor = ActivatorUtilities.CreateInstance<KnOwlPigeonOutboxInterceptor>(scope.ServiceProvider);
        ...
        Assert.Equal(envelope.Id.ToString(), result.Metadata["knowl-outbox-id"]);
        ...
        Assert.Equal("1.2.0", envelope.Metadata["knowl-pigeon-version"]);
    ...
    public async Task PigeonOutboxPublisher_replays_pigeon_publish_envelope_through_pigeon_invoker()
    {
        ...
        var publisher = provider.GetRequiredService<KnOwlPigeonOutboxPublisher>();
    ...
    public async Task PigeonOutboxPublisher_converts_legacy_payload_to_pigeon_publish_envelope()
    {
        using var provider = CreateProvider();
        var outboxSerializer = provider.GetRequiredService<IOutboxEnvelopeSerializer>();
        var legacyPayload = new KnOwlPigeonOutboxPayload
        {
        ...
        var envelope = CreateEnvelope(outboxSerializer, legacyPayload, typeof(KnOwlPigeonOutboxPayload));
        envelope.Metadata["knowl-pigeon-version"] = "1.2.0";
    ...
    private static ServiceProvider CreateProvider()
    {
        ...
        services.AddSingleton(Options.Create(new KnOwlPigeonOptions { EnableOutbox = true }));
        ...
        services.AddSingleton<KnOwlPigeonOutboxPublisher>();
        services.AddSingleton<IOutboxTransportPublisher>(
            provider => provider.GetRequiredService<KnOwlPigeonOutboxPublisher>());

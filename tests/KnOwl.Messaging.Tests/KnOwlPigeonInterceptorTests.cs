...
public sealed class KnOwlPigeonInterceptorTests
{
    [Fact]
    public async Task DecisionInterceptor_opens_inbox_and_continues_inline_consume()
    {
        ...
        var interceptor = ActivatorUtilities.CreateInstance<KnOwlPigeonDecisionInterceptor>(scope.ServiceProvider);
    ...
    public async Task ExecutionInterceptor_completes_inbox_after_successful_handler()
    {
        using var provider = CreateProvider();
        using var scope = provider.CreateScope();
        var decision = ActivatorUtilities.CreateInstance<KnOwlPigeonDecisionInterceptor>(scope.ServiceProvider);
        var execution = ActivatorUtilities.CreateInstance<KnOwlPigeonExecutionInterceptor>(scope.ServiceProvider);
        var context = CreateContext(scope.ServiceProvider);
    ...
    public async Task PigeonMuleAction_continues_inbox_invokes_consumer_and_completes_entry()
    {
        ...
        using (var workerScope = provider.CreateScope())
        {
            ...
            var action = new KnOwlPigeonMuleAction(
    ...
    private static ServiceProvider CreateProvider(Action<KnOwlPigeonOptions> configure = null)
    {
    ...
    private static KnOwlPigeonOptions CreateOptions(Action<KnOwlPigeonOptions> configure)
    {
        var options = new KnOwlPigeonOptions();
        ...
        string metadataVersion = null)
    {
        ...
        {
            Key = KnOwlPigeonMuleActionKeys.ConsumeKey,
            Metadata = System.Text.Json.JsonSerializer.Serialize(new Dictionary<string, string>
            {
                ...
                [KnOwlMuleMetadata.IdempotencyKey] = "pigeon-key"
            })
        };
        ...
        var envelope = new PigeonConsumeEnvelope
        {
            ...
            Metadata = metadataVersion is null
                ...
                : new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
                {
                    ["knowl-pigeon-version"] = metadataVersion
                }
        };

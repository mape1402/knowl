...
public sealed class InboxServiceTests
{
    ...
    public async Task OpenOrContinueAsync_uses_default_lifetime_when_configured()
    {
        ...
        var inbox = CreateInbox(new KnOwlOptions { DefaultEntryLifetime = TimeSpan.FromMinutes(5) }, clock);
    ...
    {
        var inbox = CreateInbox(new KnOwlOptions { DefaultExecutionMode = InboxExecutionMode.Deferred }, new ManualTimeProvider(DateTimeOffset.UtcNow));
    ...
    {
        var clock = new ManualTimeProvider(new DateTimeOffset(2026, 9, 9, 1, 0, 0, TimeSpan.Zero));
        var provider = CreateProvider(new KnOwlOptions { DefaultEntryLifetime = TimeSpan.FromMinutes(5) }, clock);
    ...
    private static IInboxService CreateInbox(KnOwlOptions options, TimeProvider timeProvider)
        => CreateProvider(options, timeProvider).GetRequiredService<IInboxService>();

    private static ServiceProvider CreateProvider()
        => CreateProvider(new KnOwlOptions(), new ManualTimeProvider(DateTimeOffset.UtcNow));

    private static ServiceProvider CreateProvider(Action<KnOwlOptions> configure)
        => CreateProvider(new KnOwlOptions(), new ManualTimeProvider(DateTimeOffset.UtcNow), configure);

    private static ServiceProvider CreateProvider(KnOwlOptions options, TimeProvider timeProvider)
        => CreateProvider(options, timeProvider, null);

    private static ServiceProvider CreateProvider(KnOwlOptions options, TimeProvider timeProvider, Action<KnOwlOptions> configure)
    {
        var services = new ServiceCollection();
        services.AddSingleton(timeProvider);
        services.AddKnOwl(config =>
        {

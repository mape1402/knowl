...
public sealed class KnOwlOperationServiceTests
{
    [Fact]
    ...
    private static ServiceProvider CreateProvider(Action<KnOwlOptions> configure = null)
    {
        ...
        services.TryAddEnumerable(ServiceDescriptor.Singleton<IKnOwlOperationDeferredScheduler>(scheduler));
        services.AddKnOwl(configure).UseInMemory();
    ...
    private sealed class CreateOrderOperation : KnOwlOperation<CreateOrderRequest, CreateOrderResult>
    {
    ...
    private sealed class FakeOperationScheduler : IKnOwlOperationDeferredScheduler
    {
        public List<KnOwlOperationEnvelope> Envelopes { get; } = [];

...
> Status: fulfilled by Pigeon 3.x public replay APIs and integrated by `KnOwl.Messaging.Pigeon`.

## Context

KnOwl can protect messaging consumers by opening an inbox entry before executing the protected work. For inline consumers this works naturally: a Pigeon decision interceptor opens the inbox context, the `HubConsumer` executes inside the same flow, and the inbox entry is completed or failed before the broker delivery is finished.
...
Pigeon does not need to depend on KnOwl. Pigeon needs to expose enough public infrastructure to make a consumed message replayable and invocable outside the broker receiver.
...
KnOwl would map inbox policy outcomes to these decisions:
...
KnOwl should be able to provide a Pigeon adapter that works through Pigeon's public consume pipeline:
...
- Open a KnOwl inbox entry before `HubConsumer` execution.
...
- Complete the KnOwl inbox entry when the worker succeeds.
- Fail the KnOwl inbox entry when the worker fails.
...
- Pigeon should not take a direct dependency on KnOwl.
...
Once that exists, KnOwl can provide the idempotency policy and Mule can provide durable scheduling without forcing applications to adopt another orchestration layer.

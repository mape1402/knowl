...
> Status: fulfilled by Pigeon 3.1.0. This document is kept as the integration requirement that led to the KnOwl Pigeon adapter.
...
KnOwl can use that path for deferred inbox execution.

The remaining gap is inline idempotency.

## Problem

KnOwl needs to support this inline lifecycle around a Pigeon consumer handler:
...
Without an around-handler hook, KnOwl has no clean place to complete or fail the inbox after an inline consumer finishes.
...
- require application consumers to manually complete KnOwl
...
KnOwl would use two Pigeon integration points:

1. `IConsumeDecisionInterceptor`
   - opens or continues the inbox
   - detects duplicates
   - maps KnOwl policy to `Continue`, `AckAndSkip`, `Retry`, `Reject`, or `Defer`
...
Example KnOwl execution interceptor:

```csharp
public sealed class KnOwlPigeonExecutionInterceptor : IConsumeExecutionInterceptor
{
    private readonly IInboxService _inbox;

    public KnOwlPigeonExecutionInterceptor(IInboxService inbox)
    {
        _inbox = inbox;
    }
...
- This does not require Pigeon to know anything about KnOwl.
- This does not require consumers to implement a KnOwl-specific contract.

## Why This Matters

This hook allows libraries like KnOwl to implement inline idempotency without taking over Pigeon's dispatcher or asking application code to manually complete infrastructure state.
